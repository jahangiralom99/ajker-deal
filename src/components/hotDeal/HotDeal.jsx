  ...
  return (
    <div className=" mt-10 md:bg-[#F5F5F5] bg-white border-blue-300 border-[1px] rounded-md md:border-none md:pb-5 z-10 px-2">
      <div className=" mx-auto">
        ...
        {/* tab part start */}
        <Tabs selectedIndex={tabIndex} onSelect={(index) => setTabIndex(index)} className="flex justify-center  flex-col items-center  ">
          <TabList role="tablist" className="text-center bg-transparent flex  gap-2 py-2  overflow-x-scroll">
            {grpData.map((grp, index) => (
              <Tab key={index} role="tab" className="tab md:w-20 w-16 md:h-10 text-xs border bg-[#FF8C00] rounded-md text-white   focus:outline-none focus:ring focus:ring-violet-300 ">
                {grp.name}
              </Tab>
            ))}
          </TabList>
                  ...
                  .map((itm, index) => (
                    <Link key={index} to={`/item/${itm?.item_code}`}>
                      <div className="md:border-2  md:h-56 bg-white">
                        <img className="md:w-44 md:h-40 w-20 h-22" src={`https://erp.gawsiashop.com.bd${itm?.thumbnail}`} alt="" />
                        <div className="flex justify-center items-center md:flex-col md:text-sm text-[8px]">
                          <p>
                            {itm.web_item_name.substring(0, 20)}
                            {itm.web_item_name.length > 20 ? "..." : ""}
                          </p>
